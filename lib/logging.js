(function (exports, Logger, Filter, Formatter, Handler, ConsoleHandler) {
	'use strict';
	...
	Configurator.configure = function(config, outerContext) {
		...
		for (var identifier in config.loggers) {
			...
			var settings = Object.assign({}, descriptor); // FIXME: assign
		...
		/* FIXME: Configurator._setComponents(
			params.loggers,
			instancies.loggers,
			instancies.handlers,
			'handlers',
	...
	for (var p in baseExports) {
		if (baseExports.hasOwnProperty(p)) {
			exports[p] = baseExports[p];
		}
	}
}(
	window['PyLogging'],
	window['PyLogging'].Logger,
	window['PyLogging'].Filter,
	window['PyLogging'].Formatter,
	window['PyLogging'].Handler,
	window['PyLogging'].ConsoleHandler
));