...
/%\(([a-z]+)\)(?:(\-\+|\-|\+|0| )?([0-9]+)?(?:\.([0-9]+))?(?=s|d|f|o))?([s|d|f|o])?/g;
...
Formatter.prototype.format = function(record) {
	...
	s = this._format.replace(Formatter.FORMAT_PATTERN, cb);
	if (record.error) {
...
var getPadding_cache = {};
function getPadding(count, char) {
	char = char || ' ';

	var cacheKey = char + count
	if (getPadding_cache[cacheKey]) {
		return getPadding_cache[cacheKey];
	}

	var r = (new Array(count + 1)).join(char);

	getPadding_cache[cacheKey] = r;
	return r;
}
